...
> **Controller placement clarification:** The `adapter.inbound.rest` package within `shared.fileupload` is the location for the standard's **reference implementation** controller. In a consuming application, REST controllers that compose shared services into the application's HTTP API belong in the consuming feature package (`com.example.features.<domain>.web`), not in `shared.fileupload`. The shared module exposes services, ports, and domain models — the feature owns the REST surface. If the standard provides a ready-made controller, the consuming app may use it directly or wrap it in a feature-level controller that adds application-specific logic.
...
| `hash` | `String` | SHA-256 of the stored clean-store content. On a `Sanitized` verdict (AWS profile) this is overwritten with the sanitized content's hash; otherwise it matches the uploaded content. |
| `originalHash` | `String` | SHA-256 of the originally uploaded content; populated only when the scanner verdict is `Sanitized` (null for `Unchanged` verdicts or standalone profile). Non-null value is the authoritative sanitization indicator. |
